define(["light", "triangle3D", "vector3D", "vector2D", "material", "graphics2D"], function(Light, Triangle3D, Vector3D, Vector2D, Material, Graphics2D){
    ...
    Graphics3D.prototype.fillPolygon = function(a){ 
            ...
            if (a.length%3 !== 0){
                throw "Error: Incorrect argument length in fillPolygon. Length of argument must be divisible by 3.";
            }
            if (a.length / 3 <= 2){
    ...
    Graphics3D.prototype.fillPrism = function(x, y, z, w, h, d, xr, yr, zr) { //center point x y and z - width height and depth - rotation about x, y and z axis with respect to center point
        if (typeof xr === 'undefined') { 
            ...
            yr = 0; 
            zr = 0; 
        } else if (typeof yr === 'undefined') {
            yr = 0; 
            zr = 0;
        } else if (typeof zr === 'undefined') {
            zr = 0;
        }
    ...
    Graphics3D.prototype.fillEllipsoid = function(x, y, z, xRadius, yRadius, zRadius, xr, yr, zr, divisions) {
        ...
        for (var i = 0; i <= divisions; i++){
            ...
            for (var j = 0; j <= 2 * divisions; j++){
                ...
                var rp1 = rz.multiplyVector(
                    ry.multiplyVector(
                        rx.multiplyVector(
                            p1)));
                var rp2 = rz.multiplyVector(
                    ry.multiplyVector(
                        rx.multiplyVector(
                            p2)));
                var rp3 = rz.multiplyVector(
                    ry.multiplyVector(
                        rx.multiplyVector(
                            p3)));
                var rp4 = rz.multiplyVector(
                    ry.multiplyVector(
                        rx.multiplyVector(
                            p4)));
    
                var rp5 = rz.multiplyVector(
                    ry.multiplyVector(
                        rx.multiplyVector(
                            p5)));
                var rp6 = rz.multiplyVector(
                    ry.multiplyVector(
                        rx.multiplyVector(
                            p6)));
                var rp7 = rz.multiplyVector(
                    ry.multiplyVector(
                        rx.multiplyVector(
                            p7)));
                var rp8 = rz.multiplyVector(
                    ry.multiplyVector(
                        rx.multiplyVector(
                            p8)));
